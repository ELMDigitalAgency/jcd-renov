...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mail($to, $subject, $mailMessage, $headers)) {
        echo json_encode([
            'success' => true,
            'message' => 'Merci ! Votre message a bien été envoyé. Nous vous répondrons sous peu.'
        ]);
    } else {
        // Loguer l'erreur si l'envoi échoue
        error_log("Erreur d'envoi d'email pour le contact de $email à $to");
        echo json_encode([
            'success' => false,
